...
func findLatestRunNumberReader(parentDir string) (int, error) {
...
func (r *Reader) SerializeCacheToFile() error {
	...
	latestRun, err := findLatestRunNumberReader(parentDir)
	if err != nil {
		return err
	}
	...
	fmt.Fprintln(os.Stderr, "hit_miss_rate %s", runDirPath)
	...
	fileName := fmt.Sprintf("%s/hit_miss_rate.txt", dirPath)
	fileExists := false
	if _, err := os.Stat(fileName); err == nil {
		fmt.Fprintln(os.Stderr, "hit_miss_rate File exists exiting")
		fileExists = true
		// return err
	} else {
		fmt.Fprintln(os.Stderr, "hit_miss_rate File does not exist %v moving on", err)
	}
	...
	if fileExists {
		...
		// Parse the hit and miss values from the file
		var fileHits, fileMisses int
		_, err = fmt.Sscanf(string(data), "hits = %d misses = %d", &fileHits, &fileMisses)
		if err != nil {
			fmt.Fprintln(os.Stderr, "Error parsing file data: %v", err)
			return err
		}
	...
	fmt.Fprintln(os.Stderr, "hit_miss_rate Data written")
	return nil // Return nil if no error occurs

}
func hitMissRate(r Reader) string {
	fmt.Fprintln(os.Stderr, "cacheID = %v hits = %v misses = %v", r.cacheID, r.hit.Load(), r.miss.Load())
	return fmt.Sprintf("hits = %v misses = %v", r.hit.Load(), r.miss.Load())
}

func (r *Reader) dumpData() {
	...
	if r.isDump.Load() {
		if _, err := os.Stat("/users/Khombal2/cockroach/trigger"); err == nil {
			r.Lock()
			fmt.Fprintln(os.Stderr, "hit_miss_rate Get")
			fmt.Fprintln(os.Stderr, "hit_miss_rate error in the file %v", r.SerializeCacheToFile())
			r.Unlock()
			r.isDump.Store(false)
		}
	}
}
