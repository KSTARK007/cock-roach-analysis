...
func (c *shard) evict() {
	if _, err := os.Stat("/users/Khombal2/cockroach/trigger"); err != nil {
...
func (c *Cache) Get(id uint64, fileNum base.DiskFileNum, offset uint64) Handle {
	// fmt.Fprint(os.Stderr, "cache Get")
	// f, err := os.Stat("/users/Khombal2/cockroach/trigger")
	// fmt.Fprintf(os.Stderr, "%v %v", f, err)
	// if c.Metrics().Hits%10 == 0 {
	if _, err := os.Stat("/users/Khombal2/cockroach/dump"); err == nil {
		c.isDump.Store(true)
	}
	if c.isDump.Load() {
		if _, err := os.Stat("/users/Khombal2/cockroach/trigger"); err == nil {
			c.Lock()
			fmt.Fprintln(os.Stderr, "cache Get")
			fmt.Fprintln(os.Stderr, "error in the file %v", c.SerializeCacheToFile())
			c.Unlock()
			// fmt.Fprintf(os.Stderr, "stacktrace_dump\n%s", debug.Stack())
			// err = os.WriteFile("/users/Khombal2/trace", []byte(debug.Stack()), 0644)
			// f, err := os.OpenFile("/users/Khombal2/trace", os.O_APPEND|os.O_WRONLY, 0644)
			// if err != nil {
			// 	f.WriteString(string(debug.Stack()))
			// 	f.WriteString("\n\n\n")
			// }
			// os.Exit(1)
			c.isDump.Store(false)
		}
	}
	// }
...
// SerializeCacheToFile serializes the given cache and writes the output to a file,

func (c *Cache) SerializeCacheToFile() error {
	// fmt.Fprintln(os.Stderr, "SerializeCacheToFile()")
	// Get the current process ID for unique directory naming
	processID := os.Getpid()

	// Define the parent directory
	parentDir := "/users/Khombal2/logsDump"

	// Find the latest run number
	latestRun, err := findLatestRunNumber(parentDir)
	if err != nil {
		return err
	}
	...
	runNumber := latestRun
	if runNumber == 0 {
		// If no run directory exists, start with run1
		runNumber = 1
	}

	// Create the run directory path
	runDirPath := fmt.Sprintf("%s/run%d", parentDir, runNumber)
	fmt.Fprintln(os.Stderr, "%s", runDirPath)
	// Ensure the run directory exists
	err = os.MkdirAll(runDirPath, 0777)
	if err != nil {
		fmt.Fprintln(os.Stderr, "MkdirAll result path error")
		return err // Return the error if run directory creation fails
	}

	// Create a directory path including the process ID
	dirPath := fmt.Sprintf("%s/%d", runDirPath, processID)
	fmt.Fprintln(os.Stderr, "%s", dirPath)
	// Ensure the process ID directory exists
	err = os.MkdirAll(dirPath, 0777)
	if err != nil {
		fmt.Fprintln(os.Stderr, "MkdirAll dirPath error")
		return err // Return the error if process ID directory creation fails
	}

	// Create a timestamped file name, formatted to the day
	fileName := fmt.Sprintf("%s/cachedump.txt", dirPath)
	if _, err := os.Stat(fileName); err == nil {
		fmt.Fprintln(os.Stderr, "File exists exiting")
		return err
	} else {
		fmt.Fprintln(os.Stderr, "File does not exist %v moving on", err)
	}
	// Serialize the cache
	output := serializeCache(*c)

	// fmt.Fprintln(os.Stderr, "%s", output)
	// Write the output to a file in the process ID directory, overwriting it if it exists
	...
	fmt.Fprintln(os.Stderr, "Data written")
	return nil // Return nil if no error occurs
}

// Helper functions used within SerializeCacheToFile:

func serializeCache(cache Cache) string {
	fmt.Fprintln(os.Stderr, "serializeCache()")
	var result string
	shard_count := 0
	for _, shard := range cache.shards {
		shard_count++
		// shard.mu.Lock()
		fmt.Fprintln(os.Stderr, "shard id = %d", shard_count)
		result += fmt.Sprintf("Shard hits = %v misses = %v {\n", shard.hits.Load(), shard.misses.Load())
		for _, entry := range shard.blocks.getEntries() {
			result += "  Block Entry: " + serializeEntry(entry.value) + "\n"
		}
		// for _, entry := range shard.files.getEntries() {
		// 	result += "  File Entry: " + serializeEntry(entry.value) + "\n"
		// }
		result += "}\n"
		// shard.mu.Unlock()
	}
	result += fmt.Sprintf("shard_count = %d", shard_count)
	result += fmt.Sprintf("Metrics = %+v", cache.Metrics())
	return result
}

func serializeEntry(e *entry) string {
	if e == nil {
		return "nil"
	}
	if e.acquireValue() == nil {
		return "nil"
	}
	// if e.referenced.Load() == false {
	// 	e.referenced.Store(true)
	// 	tmp := fmt.Sprintf("Entry{value:%s, size: %d, ptype: %d, referenced: %t}",
	// 		string(e.acquireValue().buf[:]), e.size, e.ptype, e.referenced.Load())
	// 	e.referenced.Store(false)
	// 	return tmp
	// }
	// return fmt.Sprintf("Entry{value: %s, size: %d, ptype: %d, referenced: %t}",
	// 	string(e.val.buf), e.size, e.ptype, e.referenced.Load())
	// fmt.Fprintln(os.Stderr, "serializeEntry() %v", e.key)
	tmp := fmt.Sprintf("Entry{value:%s, size: %d, ptype: %d, referenced: %t}",
		string(e.acquireValue().buf[:]), e.size, e.ptype, e.referenced.Load())
	// fmt.Fprint(os.Stderr, "%s", tmp)
	return tmp
	// return tmp

}

func (r robinHoodMap) getEntries() []robinHoodEntry {
	entries := make([]robinHoodEntry, r.entries.len)

	for i := uint32(0); i < r.entries.len; i++ {
		entry := r.entries.at(i)
		if entry != nil { //&& entry.value.ref.Load() != 0 {
			entries[i] = *entry
		}
	}
...
func findLatestRunNumber(parentDir string) (int, error) {
	// Read all entries in the parent directory
	entries, err := ioutil.ReadDir(parentDir)
	if err != nil {
		return 0, err // Return an error if unable to read the directory
	}

	maxRunNumber := 0
	for _, entry := range entries {
		if entry.IsDir() {
			// Check if the directory name starts with "run"
			dirName := entry.Name()
			if strings.HasPrefix(dirName, "run") {
				// Extract the number part from the directory name
				numberStr := dirName[3:]
				number, err := strconv.Atoi(numberStr)
				if err == nil && number > maxRunNumber {
					maxRunNumber = number
				}
			}
		}
	...
	return maxRunNumber, nil // Return the highest run number found, or 0 if none found
}
